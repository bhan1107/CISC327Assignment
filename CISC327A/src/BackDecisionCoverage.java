...
public class BackDecisionCoverage {
	...
	public void handleCreateService(Transaction transaction) {
		...
		// if createservice is valid then create new CSF object
		if(index == -1) { 
	...
	public void handleSellTicket(Transaction transaction) {
		...
		if (!(transaction.numTickets <= 0) && !(transaction.numTickets > capacity) && capacityConstraint(capacity)) {
			service.serviceCapacity -= transaction.numTickets;
	...
	public void handleCancelTicket(Transaction transaction) {
		...
		if (index != -1) {
			...
			if (service.numTickets - transaction.numTickets >= 0 && service.numTickets > 0) {
				System.out.println("service.numTickets - transaction.numTickets is greater than 0");
				service.numTickets -= transaction.numTickets;
			}
			else {
				System.out.println("service.numTickets - transaction.numTickets is less than 1");
			}
		}
		else {
			System.out.println("index is -1");
	...
	public int checkServiceNumber(int serviceNumber) { //returns index of location where service number is the same
		...
		for(int i=0;i<CSF.size();i++) {
			if(CSF.get(i).serviceNum == serviceNumber) {
				System.out.println("serviceNumber was found in the CSF");
				return i;
			}
			else {
				System.out.println("serviceNumber was not found in the CSF");
			}
		}
		return -1;
	}
